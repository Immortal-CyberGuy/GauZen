    ...
    try {
      ...
      setError(
        err.message.includes("Network") || err.message.includes("fetch")
          ? "Network error. Check if backend server is reachable and CORS is allowed."
          : err.message
      );
      // Optional fallback to mock data:
      // setVetDoctors(mockVetData);
    } finally {
      setIsLoading(false);
    }
  };
          ...
          {selectedVet && (
            <InfoWindow position={{ lat: selectedVet.geometry.location.lat, lng: selectedVet.geometry.location.lng }} onCloseClick={() => setSelectedVet(null)}>
              <div>
                ...
                {selectedVet.formatted_phone_number && (
                  <p>
                    <strong>Phone:</strong> <a href={`tel:${selectedVet.formatted_phone_number}`}>{selectedVet.formatted_phone_number}</a>
                  </p>
                )}
              </div>
            </InfoWindow>
